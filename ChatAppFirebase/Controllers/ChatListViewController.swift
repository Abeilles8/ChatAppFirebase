...
//  ChatAppFirebase
//
//  Created by Yuta Uematsu on 2020/05/01.
...
class ChatListViewController: UIViewController {
  ...
  private func handleAddedDocumentChange(documentChange: DocumentChange) {
    ...
    chatroom.memebers?.forEach { (memeberUid) in
      if memeberUid != uid {
        Firestore.firestore().collection("users").document(memeberUid).getDocument { (userSnapshot, err) in
          if let err = err {
  ...
  private func pushLoginViewController() {
    ...
    let signUpViewController = storyboar.instantiateViewController(identifier: "SignUpViewController") as! SignUpViewController
